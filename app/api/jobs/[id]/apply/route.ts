  ...
  try {
    ...
    if (existingApplication) {
      ...
      return NextResponse.redirect(
        new URL(`/jobs/${params.id}?applied=1`, req.url)
      );
    }
    ...
    if (!resume) {
      ...
      return NextResponse.redirect(
        new URL(`/jobs/${params.id}?applied=resume_missing`, req.url)
      );
    }
    ...
    return NextResponse.redirect(
      new URL(`/jobs/${params.id}?applied=success`, req.url)
    );
  } catch (error) {
    console.error('Error applying for job:', error);
    // Redirect to job page with error
    return NextResponse.redirect(
      new URL(`/jobs/${params.id}?applied=error`, req.url)
    );
  }
}
